...
interface LocalBusinessSearchProps {
  onSearch: (industry: string, location: string) => void
  loading?: boolean
  ...
  return (
    <div className="border-b border-zinc-800 bg-gradient-to-r from-indigo-500/5 to-transparent px-4 py-4 sm:px-6">
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 lg:flex-row lg:items-end">
        <div className="flex flex-1 flex-col gap-3 sm:flex-row">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Industry (dentists, gyms, roofing…)"
              value={industry}
        ...
        {SUGGESTED_SEARCHES.map((s) => (
          <button
            ...
            onClick={() => {
              ...
              onSearch(s.industry, s.location)
            }}
      ...
      <p className="mt-2 text-xs text-zinc-600">
        AI outbound reconnaissance — Google Places when configured, with opportunity scoring on every result.
      </p>
    </div>
  )
}
