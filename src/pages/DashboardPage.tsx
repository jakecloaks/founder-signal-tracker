...
export function DashboardPage() {
  ...
  const topOutreach = useMemo(() =>
    [...businesses].sort((a, b) => b.fitScore - a.fitScore).slice(0, 4).map((b) => ({
      id: b.id, name: b.name, angle: b.outreachAngle, fitScore: b.fitScore,
      contactMethod: b.bestContactMethod, business: b,
    })),
    [businesses],
  )
    ...
    <div className="flex min-h-screen bg-[#0C0C10] text-[#EAEAF0]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="flex items-center gap-2 border-b border-[#1F1F30] bg-[#101015] px-4 py-3 md:hidden">
          ...
          <Link to="/" className="ml-auto text-xs text-[#424258] hover:text-[#82829A] transition-colors">Home</Link>
        </div>
        ...
        <main className="flex-1 overflow-auto">
          ...
          {!scanning && businesses.length > 0 && (
            <div className="flex items-center overflow-x-auto border-b border-[#1F1F30] bg-[#101015]">
              {[
                { icon: Building2, label: 'Scanned',       value: String(stats.total),    color: 'text-[#EAEAF0]' },
                { icon: Flame,     label: 'High fit ≥70',  value: String(stats.hot),      color: 'text-[#E07A45]' },
                { icon: Target,    label: 'Avg fit',        value: String(stats.avg),      color: 'text-[#4A8FE0]' },
                ...(opportunityValue.min > 0 ? [{
                  icon: DollarSign, label: 'Est. opportunity',
                    ...
                    return (
                      <button
                        ...
                        className={`rounded px-2.5 py-1.5 font-medium transition-colors ${
                          active ? 'bg-[#4A8FE0]/10 text-[#4A8FE0]' : 'text-[#424258] hover:text-[#82829A]'
                        }`}
                      >
                        {label}
                      </button>
                    ...
                    <UpgradeGate
                      ...
                      lockedPreviews={
                        <div>{lockedResults.slice(0, 3).map((b, i) => (
                          <BusinessCard key={b.id} business={b} listStyle index={i} onClick={() => {}} />
                        ))}</div>
                      }
                    />
                  )}
                      ...
                      lockedPreviews={
                        <div className="grid gap-3 px-5 pb-2 sm:grid-cols-2">
                          {lockedResults.slice(0, 4).map((b, i) => (
                            <BusinessCard key={b.id} business={b} index={i} onClick={() => {}} />
                          ))}
                        </div>
                      }
                    />
            ...
            <div className="hidden p-4 xl:block xl:space-y-4">
                    ...
                    {topOutreach.map((item) => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <span className="truncate text-xs font-semibold text-[#EAEAF0]">{item.name}</span>
                          <span className="shrink-0 text-xs font-bold tabular-nums text-[#E07A45]">{item.fitScore}</span>
                        </div>
                        <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-[#82829A]">{item.angle}</p>
              ...
              {businesses.length > 0 && (
                <div className="overflow-hidden rounded-xl border border-[#1F1F30] bg-[#16161D]">
                  ...
                  <div className="divide-y divide-[#1A1A28]">
                    {[
                      { label: 'No website',       value: businesses.filter((b) => !b.footprint.websiteExists).length,  color: 'text-[#D95555]' },
                      { label: 'Weak social',       value: businesses.filter((b) => !b.footprint.instagramExists || b.footprint.instagramActivityScore < 40).length, color: 'text-[#E8A520]' },
                      { label: '★4.5+ weak digital', value: businesses.filter((b) => b.googleRating >= 4.5 && b.footprint.digitalPresenceStrength < 45).length, color: 'text-[#E07A45]' },
                    ].map((row) => (
                      <div key={row.label} className="flex items-center justify-between px-4 py-2.5">
                        <span className="text-xs text-[#82829A]">{row.label}</span>
      ...
      {showUpgradeModal && (
        <UpgradeModal
          ...
          onUpgrade={() => { credits.addCredits(50); setShowUpgradeModal(false) }}
        />
      )}
    </div>
  )
